...
│   ├── data_sources.yaml         # Dual-source schema: PRDECT-ID (A) + Tokopedia 2019 (B)
│   ├── experiment_settings.yaml  # Phase 4 experiment params, split policy, metrics
│   └── project_settings.yaml     # Phase flag, version, governed paths, logging
│
├── data/                         # Governed multi-tier data directory (LOCAL_ONLY except README/.gitkeep)
│   ├── README.md                 # Data governance & tier semantics definition
│   ├── raw/                      # Immutable raw source data landing (Gitignored)
│   ├── interim/                  # Standardized corpora, split indices (Gitignored)
│   ├── processed/                # Model-ready features (Gitignored)
│   └── metadata/                 # Source manifests, SHA checksums, capability matrix, lineage
│
├── docs/                         # Governed project documentation (canonical truth)
│   ├── architecture/             # Phase 5: solution arch, data arch, dim model, contracts
│   ├── engineering/              # Technical env, repository structure docs
│   ├── governance/               # Charter, policies, risk, DoD, gates, synthetic policy
│   ├── methodology/              # Phase 4: analytical design, experiment + evaluation protocol
│   ├── plans/                    # Phase implementation plans & schedule artefacts
│   ├── requirements/             # Phase 3: BRD, SRS, use cases, MVP, RTM (v2.0)
│   └── research/                 # Background, competitor, domain, prior-art references
│
├── logs/                         # Runtime output artefacts (LOCAL_ONLY; *.log Gitignored)
│
├── reports/                      # System validation reports & portfolio outputs
│   └── validation/               # Phase 01–05 validation + forensic audit + remediation
│
├── scripts/                      # Operational automation utilities
│   ├── data_acquisition/         # Dataset registration, SHA manifest generation
│   ├── data_audit/               # Phase 2 forensic audit + hardening scripts
│   ├── environment/              # Environment health & reproducibility validators
│   ├── methodology/              # Phase 4 YAML/design/evidence validators
│   └── requirements/             # Phase 3 RTM & requirement validators
│
...
│
└── tests/                        # Automated test suite (stdlib unittest + pytest)
    ├── __init__.py
